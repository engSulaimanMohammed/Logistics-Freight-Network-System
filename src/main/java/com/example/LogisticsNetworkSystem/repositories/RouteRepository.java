...
public interface RouteRepository extends JpaRepository<Route, Long> {
    @Query(value = "SELECT r FROM Route r WHERE r.driver.id = :driverId AND r.routeDate = :routeDate AND r.isActive = true")
    List<Route> findRoutesForDriverOnDate(
            @Param("driverId") Long driverId,
            @Param("routeDate") Date routeDate
    );

    @Query(value = "SELECT COUNT(r) FROM Route r WHERE r.vehicle.carrier.id = :carrierId AND r.isActive = true")
